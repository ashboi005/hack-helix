...
export function ProjectHomeHub() {
  ...
  const launchOptions: LaunchOption[] = [
    ...
    isAuthenticated
      ? {
        ...
        description: "You are signed in and your account is active.",
  ...
  return (
    <main className={`${spaceGrotesk.className} relative min-h-screen overflow-hidden bg-[#040812] text-zinc-100`}>
      ...
      <section className="relative mx-auto flex min-h-screen w-full max-w-[1800px] flex-col px-4 py-8 sm:px-5 lg:px-6">
        ...
        <div className="grid flex-1 auto-rows-fr gap-4 md:grid-cols-2 xl:grid-cols-2">
          {launchOptions.map((option) => {
              ...
              <>
                ...
                <div className="relative flex h-full w-full flex-col justify-between gap-6">
                  <div className="space-y-4">
                    ...
                    {option.info && <p className="text-xs font-mono text-zinc-300/90">{option.info}</p>}
                  </div>
                  {option.actionLabel && (
            ...
            return (
              <article
                key={option.title}
                className="group relative flex min-h-[200px] overflow-hidden rounded-2xl border border-white/10 bg-[#070e1a]/90 p-6 shadow-[0_25px_40px_-28px_rgba(0,0,0,0.95)] md:p-8"
              >
                {content}
              </article>
            )
