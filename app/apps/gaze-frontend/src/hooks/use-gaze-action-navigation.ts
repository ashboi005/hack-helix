...
function readCalibrationReady() {
    try {
        ...
        return Boolean(calibrationRecord?.calibration && calibrationRecord.neutralSnapshot)
    } catch {
        return false
    }
}
...
export function useGazeActionNavigation(options: UseGazeActionNavigationOptions = {}) {
    ...
    // Fetch eye token when auth session is available
    useEffect(() => {
    ...
    // Reset recovery flag when websocket connects successfully
    useEffect(() => {
    ...
    // Auto-recover from websocket auth errors by re-fetching token (once)
    useEffect(() => {
    ...
    // Sync calibration-ready state from localStorage
    useEffect(() => {
    ...
    // Auto-start camera preview when gaze is enabled
    useEffect(() => {
        if (!gazeControlEnabled) return
    ...
    // Auto-start live preview websocket once camera preview is running
    useEffect(() => {
        if (!gazeControlEnabled) return
        if (!gazeState.previewActive) return
    ...
    // Track which action element the gaze is focused on
    useEffect(() => {
    ...
    // Handle Space key to trigger focused action elements
    useEffect(() => {
        if (!gazeControlEnabled) return
